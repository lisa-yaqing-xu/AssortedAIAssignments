...
class ConnectFourBoard(object):
    ...
    def __init__(self, winning_streak = None,game_mode = None,board_array = None, board_already_won = None, modified_column = None, current_player = 1, previous_move = -1):
        ...
        if(winning_streak != None):
            self.win_streak = winning_streak;
        if(game_mode != None):
            self.game_mode = game_mode;
            if(self.game_mode == "longest"):
                self.win_streak = max(self.board_width, self.board_height);
    ...
    def do_move(self, column):
        ...
        return ConnectFourBoard(board_array=new_board,winning_streak=self.win_streak,game_mode=self.game_mode,board_already_won=self.is_win(), modified_column=column, current_player = self.get_other_player_id())
    ...
    def _is_win_from_cell(self, row, col):
        ...
        return ( self._max_length_from_cell(row, col) >= self.win_streak )
    ...
    def clone(self):
        ...
        return ConnectFourBoard(winning_streak=self.win_streak,game_mode=self.game_mode,board_array=self._board_array,board_already_won=self._is_win, current_player = self.get_current_player_id())
...
class ConnectFourRunner(object):
    ...
    * The game ends when one of the two players has four consecutive tokens in a row
    ...
    def run_longestss(self, verbose=True):
        """ Run the test defined by this test runner.  Print and return the id of the winning player. """
        player1 = (self.player1_callback, 1, self._board.board_symbol_mapping[1])
        player2 = (self.player2_callback, 2, self._board.board_symbol_mapping[2])
        ...
        if win_for_player != 0 and self._board.is_tie():
            ...
            self._do_gameend(win_for_player)
            return win_for_player
    def run_longest(self, verbose=True):
        """ Run the test defined by this test runner.  Print and return the id of the winning player. """
        player1 = (self.player1_callback, 1, self._board.board_symbol_mapping[1])
        player2 = (self.player2_callback, 2, self._board.board_symbol_mapping[2])
                ...
                while not has_moved:
                    try:
                        ...
                        if sys.stdout.encoding and 'UTF' in sys.stdout.encoding:
                            ...
                            continue
                moves -= 1
        if self._board.longest_chain(1) > self._board.longest_chain(2):
            win_for_player = 1
        elif self._board.longest_chain(1) < self._board.longest_chain(2):
            win_for_player = 2
...
# modified here to take an argument of win streak with the default being 4
def run_game(player1, player2,board = ConnectFourBoard()):
    ...
    if(board.game_mode == "classic"):
        return game.run_game()
    else:
        return game.run_longest()
